...
# ASCEND_INSTALL_PATH: The installation path of the CANN package, where "/usr/local/Ascend/ascend-toolkit/latest/" 
#                  is the default installation path. If user defines the installation path, please modify it.
# Uncomment and modify it when you specified installation path of CANN.
...
# COMPILE_KERNEL_TARGET: The option of the custom operator compile kernels target, which "cpu_tbe" is the
#                      default compile option. If user defines the compile option, please modify it.
# Uncomment and modify it when you specified the compile option.
...
# Create version.info
compiler_version=$(grep "Version" -w ${ASCEND_INSTALL_PATH}/compiler/version.info | awk -F = '{print $2}')
echo "custom_opp_compiler_version=${compile_version}" > ${project_path}/build_out/version.info
