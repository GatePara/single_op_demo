...
该样例实现了对[AddBlockCust](../../1_custom_op/doc/AddBlockCust_CN.md)的功能验证，通过将自定义算子转换为单算子离线模型文件，然后通过AscendCL加载单算子模型文件进行运行。
...
│               └── addblockcust_op.json    // 算子描述文件，用于构造单算子模型文件
│           ├── data
...
- 操作系统及架构：CentOS x86\_64、CentOS aarch64、EulerOS x86、EulerOS aarch64、Ubuntu 18.04 x86\_64
...
1.  生成AddBlockCust算子的单算子离线模型文件。
    1.  以运行用户（例如HwHiAiUser）登录开发环境，并进入样例工程的“acl\_execute\_addblockcust/run/out“目录。
...
4. 在硬件设备的Host侧执行单算子验证文件。
    1.  以运行用户（例如HwHiAiUser）拷贝开发环境中样例工程acl\_execute\_addblockcust/run/目录下的out文件夹到运行环境任一目录，例如上传到/home/HwHiAiUser/HIAI\_PROJECTS/run\_addblockcust/目录下。

        **说明：** 若您的开发环境即为运行环境，此拷贝操作可跳过。

    2.  在运行环境中执行execute\_custom\_add\_block\_cust\_op文件，验证单算子模型文件。

        在/home/HwHiAiUser/HIAI\_PROJECTS/run\_addblockcust/out目录下执行如下命令：
